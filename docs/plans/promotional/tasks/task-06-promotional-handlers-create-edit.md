# Task 6: Promotional Handlers — Create + Edit

**Files:**
- Modify: `handlers/promotional.go`

**Interfaces:**
- Consumes: `queries.GetAllPromotionals()`, `queries.GetPromotionalByID()`, `queries.CreatePromotional()`, `queries.UpdatePromotional()`, `queries.SoftDeletePromotional()`
- Produces: `AddPromotional(c *fiber.Ctx) error`, `EditPromotionalGet(c *fiber.Ctx) error`, `EditPromotionalPut(c *fiber.Ctx) error`, `DeletePromotional(c *fiber.Ctx) error`

**Steps:**

- [ ] **Step 1: Update the handler file**

Replace the entire content of `handlers/promotional.go` with:
